...
class BrunoApi{
	...
	*/
	public function galeria(WP_REST_Request $request){
		...
		foreach ($media_query->posts as $post) {
			$foto = array(
				'thumb' => wp_get_attachment_image_src($post->ID)[0],
				'src' => wp_get_attachment_image_src( $post->ID, 'full_galeria' )[0],
				'sub' => get_the_title($post->ID)
			);
	...
	*/
	public function comentar(WP_REST_Request $request) {
		...
		$user_id = get_current_user_id();
		$user_data = get_user_by('id', $user_id);
		$assunto   = $user_data->display_name.' enviou um comentário para o fórum';
		$mensagem = '';
		$mensagem .= '<h3>Parcicipante</h3>';
		$mensagem .= 'De: '. $user_data->display_name.'<br />';
		$mensagem .= 'E-mail: '. $user_data->user_email.'<br />';
