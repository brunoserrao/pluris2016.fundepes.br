 ...
 */
function pluris2016_customize_register( $wp_customize ) {
	...
	$wp_customize->add_setting( 'color_scheme', array(
		'default'           => 'default',
		'sanitize_callback' => 'pluris2016_sanitize_color_scheme',
		'transport'         => 'postMessage',
	) );

	$wp_customize->add_control( 'color_scheme', array(
		'label'    => __( 'Base Color Scheme', 'pluris2016' ),
		'section'  => 'colors',
		'type'     => 'select',
		'choices'  => pluris2016_get_color_scheme_choices(),
		'priority' => 1,
	) );
	...
	$wp_customize->add_control( new WP_Customize_Color_Control( $wp_customize, 'sidebar_textcolor', array(
		'label'       => __( 'Header and Sidebar Text Color', 'pluris2016' ),
		'description' => __( 'Applied to the header on small screens and the sidebar on wide screens.', 'pluris2016' ),
		'section'     => 'colors',
	) ) );
	...
	$wp_customize->add_control( new WP_Customize_Color_Control( $wp_customize, 'header_background_color', array(
		'label'       => __( 'Header and Sidebar Background Color', 'pluris2016' ),
		'description' => __( 'Applied to the header on small screens and the sidebar on wide screens.', 'pluris2016' ),
		'section'     => 'colors',
	) ) );

	// Add an additional description to the header image section.
	$wp_customize->get_section( 'header_image' )->description = __( 'Applied to the header on small screens and the sidebar on wide screens.', 'pluris2016' );
}
add_action( 'customize_register', 'pluris2016_customize_register', 11 );

/**
 * Register color schemes for Twenty Fifteen.
 *
 * Can be filtered with {@see 'pluris2016_color_schemes'}.
 *
 ...
 */
function pluris2016_get_color_schemes() {
	/**
	 ...
	 * }
	 */
	return apply_filters( 'pluris2016_color_schemes', array(
		'default' => array(
			'label'  => __( 'Default', 'pluris2016' ),
			'colors' => array(
				'#f1f1f1',
				'#ffffff',
				'#ffffff',
...
if ( ! function_exists( 'pluris2016_get_color_scheme' ) ) :
/**
 ...
 */
function pluris2016_get_color_scheme() {
	$color_scheme_option = get_theme_mod( 'color_scheme', 'default' );
	$color_schemes       = pluris2016_get_color_schemes();
...
endif; // pluris2016_get_color_scheme

if ( ! function_exists( 'pluris2016_get_color_scheme_choices' ) ) :
/**
 ...
 */
function pluris2016_get_color_scheme_choices() {
	$color_schemes                = pluris2016_get_color_schemes();
...
endif; // pluris2016_get_color_scheme_choices

if ( ! function_exists( 'pluris2016_sanitize_color_scheme' ) ) :
/**
 ...
 */
function pluris2016_sanitize_color_scheme( $value ) {
	$color_schemes = pluris2016_get_color_scheme_choices();
...
endif; // pluris2016_sanitize_color_scheme
 ...
 */
function pluris2016_color_scheme_css() {
	$color_scheme_option = get_theme_mod( 'color_scheme', 'default' );
	...
	$color_textcolor_rgb         = pluris2016_hex2rgb( $color_scheme[3] );
	$color_sidebar_textcolor_rgb = pluris2016_hex2rgb( $color_scheme[4] );
	$colors = array(
	...
	$color_scheme_css = pluris2016_get_color_scheme_css( $colors );

	wp_add_inline_style( 'pluris2016-style', $color_scheme_css );
}
add_action( 'wp_enqueue_scripts', 'pluris2016_color_scheme_css' );
 ...
 * @since Twenty Fifteen 1.0
 */
function pluris2016_customize_control_js() {
	...
	wp_localize_script( 'color-scheme-control', 'colorScheme', pluris2016_get_color_schemes() );
}
add_action( 'customize_controls_enqueue_scripts', 'pluris2016_customize_control_js' );
 ...
 * @since Twenty Fifteen 1.0
 */
function pluris2016_customize_preview_js() {
	wp_enqueue_script( 'pluris2016-customize-preview', get_template_directory_uri() . '/js/customize-preview.js', array( 'customize-preview' ), '20141216', true );
}
add_action( 'customize_preview_init', 'pluris2016_customize_preview_js' );
 ...
 */
function pluris2016_get_color_scheme_css( $colors ) {
 ...
 * @since Twenty Fifteen 1.0
 */
function pluris2016_color_scheme_css_template() {
	$colors = array(
	...
	?>
	<script type="text/html" id="tmpl-pluris2016-color-scheme">
		<?php echo pluris2016_get_color_scheme_css( $colors ); ?>
	</script>
	<?php
}
add_action( 'customize_controls_print_footer_scripts', 'pluris2016_color_scheme_css_template' );
